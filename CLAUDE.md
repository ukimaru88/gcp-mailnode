...
> 最后更新：2026-05-30
> 当前版本：**v0.2.24**（`version.txt`；亚洲 7 区域多选并发筛 IP，worker 固定绑定 region 各自撑满池）
