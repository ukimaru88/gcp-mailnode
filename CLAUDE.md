...
> 最后更新：2026-06-20
> 当前版本：**v0.2.38**（`version.txt`；修 Postfix 提取退出码丢数据 + relay=local 污染 + SSH 脚本审计）
...
| 版本 | 改动 |
|---|---|
| **v0.2.38** | 最新。**两个真 bug + 6 文件 SSH 脚本审计**：用户 v0.2.37 提取仍失败，贴出节点 mail.log。**Bug 1（退出码丢数据）**：app_extract.go 的 Postfix 脚本结尾 `for f in /var/log/mail.log.*.gz; do [ -f "$f" ]&&zcat; done`，无 .gz 文件时 bash 空 glob 不展开成字面量 → `[ -f "/var/log/mail.log.*.gz" ]` 返回 1 → 整脚本退出码=1（`set +e` 不改最终退出码）→ ssh.RunCommand 判失败 → 调用方 `if err!=nil{return}` 把**已拿到的完整日志全丢了**。用户日志里明明有 `status=sent (250 Mail accepted)`，纯被退出码坑。修：脚本结尾 `exit 0` + content 非空就解析（双保险）。**Bug 2（relay=local 数据污染）**：parsePostfix 只要匹配 `status=sent` 就提邮箱，没排除 `relay=local`。Postfix 每收一封外部退信会产生一条退信回执投到 `info@本域`（relay=local, status=sent, delivered to maildir）——这不是真收件人，会污染提取结果。修：sent 分支加 `relay=local` 排除，只提 relay=外部MX 的。新增 maillog_test.go 回归测试（用用户真实日志样本验证 relay=local 排除 + 外部 sent 提取）。**Workflow 审计**：6 agent 并行审 app_extract/app_server_status/stages/client/orchestrator/app_kumomta_diag 全部 SSH 脚本调用，确认只 client.go ReadMailLogFull 有同款退出码陷阱（s6 分支 while 末尾 cat 失败丢已拼数据），顺手加 `exit 0` + content 非空容错；其余全 none（awk exit 0 / `\|\| true` 兜底 / `exit $fail` 是设计意图）。go test ./... 全绿 |
| v0.2.37 | **邮箱提取加 KumoMTA/Postfix 手动开关**：用户反馈 v0.2.36 还是不行——截图所有节点报"读取 KumoMTA 日志目录失败 /var/log/kumomta 不存在"。根因数据库 `vps_instances.deploy_type` 字段对老节点不正确（v0.2.10 之前部署没设置默认 'kumomta'，或者部署 Postfix 但字段没改），导致前端 `isExtractableVPS` 也只过滤 kumomta 类型，强走 KumoMTA 路径。改：① 后端加 `ExtractFromVPSForceType(vpsIDs, deleteAfter, forceType)` Wails 方法，forceType='kumomta'/'postfix' 强制覆盖 DB 字段，extractCore 用 effType 决定走哪个分支 ② 前端 Extract.tsx 加 Toggle 开关（KumoMTA ↔ Postfix）+ 颜色区分（紫色/琥珀色），切到 Postfix 时"删服务器日志"自动 disable 灰掉 ③ isExtractableVPS 放宽：kumomta + postfix + 空 deploy_type 都列出来 ④ run 调用统一切到 ExtractFromVPSForceType，按开关传 forceType ⑤ 日志加 `type=DB(eff=有效)` 方便对账 |
| v0.2.36 | **修 v0.2.35 Postfix 兜底命令 + install_postfix 装 rsyslog**：用户反馈 v0.2.32 部署的 Postfix 节点用 v0.2.35 提取还是 0 邮箱。根因 install_postfix.sh 没装 rsyslog（Debian 12 minimal 默认不带），/var/log/mail.log 不存在；v0.2.35 兜底 `journalctl -u postfix` **是错的**——Postfix 不是 systemd unit 直接输出，是 master + smtp + smtpd + cleanup + qmgr + bounce 等子进程各自通过 syslog 写日志，SYSLOG_IDENTIFIER 是 'postfix/smtp' 这种格式（status=sent 来自 postfix/smtp，status=bounced 来自 postfix/bounce）。修：① app_extract.go 兜底改 `journalctl -t postfix/smtp -t postfix/smtpd -t postfix/cleanup -t postfix/bounce -t postfix/error -t postfix/local -t postfix/qmgr -t postfix/pickup` 拿全所有 postfix 子进程的 syslog ② install_postfix.sh 加 rsyslog 包 + systemctl enable + start（新部署自动有 mail.log，老节点靠 journalctl 兜底）。**老节点不用重装**，换 v0.2.36 软件就能提取出来 |
| v0.2.35 | **邮箱提取支持 Postfix**：用户反馈"选 Postfix 部署的节点提取不出来发送成功"。根因 app_extract.go 只对 mailcow 单独处理（标 error），Postfix 直接走 KumoMTA 路径——读 /var/log/kumomta/（Postfix 没这目录）+ ParseKumoMTAStream（Postfix 是 syslog 文本不是 JSON），双错；自动调度 SQL where 也只查 kumomta 类型，Postfix 节点连自动任务列表都进不去。改动：① ssh 拉日志按 deploy_type 分支——postfix → cat /var/log/mail.log + 历史轮转（.1/.2/*.gz）+ journalctl 兜底（Debian 12 minimal 没 rsyslog 时） ② 解析器对应切 parser.ParseMailLog（已有 Postfix regex 支持 to=<email> + status=sent/bounced/deferred） ③ 自动调度 SQL `IN ('kumomta','postfix')` ④ Postfix 暂不支持 deleteAfter（syslog 不能按 cursor 删，需 logrotate 配合）。前后端编译 0 错 + parser 单测过 + 打包 |
| v0.2.33 | **「连接服务器」加深度诊断**：用户反馈 26863 队列堵塞但只看到 Bounce 5xx 统计不知道为什么堵。新增 4 个 Panel：① 临时失败原因 (Deferred 4xx) 分类 + Top 域名（区别 Bounce，看真正堵塞原因） ② 最近 30 条 SMTP 响应原文（带时间/域名/收件人脱敏，看对方服务器返回原话）③ kcli queue-summary 队列堵塞 Top 域名（实时快照不是日志统计）④ queue-summary 原文。后端 ServerStatusDTO 加 DeferredReasons / RecentSmtpReplies / QueueSummary / TopQueueDomains；serverStatusCommand 加 kcli 路径自动发现（/opt/kumomta/{sbin,bin}/ + PATH） + base64 传输避免特殊字符破坏 KV 解析；applyKumoLogStats 加 TransientFailure 事件分类 + 收集真实 SMTP 响应。kumoTimestamp + maskRecipient 工具函数。前端 ServerStatus.tsx 加 3 个 Panel（临时失败琥珀色、SMTP 响应原文按 Bounce/Deferred 着色、队列堵塞 Top 玫红色）。前后端编译 0 错，打包 |
